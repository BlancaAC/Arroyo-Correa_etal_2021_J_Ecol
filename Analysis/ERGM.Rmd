...
rocina <- rocina[,c(5,7,8)]

rocina.m <- dcast(rocina, Plant_ID ~ Pollinator_fg, value.var="N_visitations", fun.aggregate = sum)
...
x <- data %>%  
           dplyr::select(c(12:31))  # Select predictor variables
str(x)
...
# ERGMs with bipartite networks

## Set node attibutes

```{r nodes bi, echo= TRUE, message= TRUE, warning= FALSE, fig.width= 9}
 
# Add the node attributes -> visits 2:9
...
First, I separated the network into adjacency matrix and attributes. Then, I create the analogous network object for ergm and set vertex (i.e. node) attributes in the network object.


```{r Bernoulli bi, echo=TRUE, message= TRUE, warning= TRUE, fig.width=9}
...
```{r models fit bi, echo=TRUE, message= TRUE, warning= TRUE}

ergm1 <- formula(int.s ~ edges  
                        + nodemain("Plant_ID")
...
```{r model tests bi, echo=TRUE, message= FALSE, warning= TRUE}

# Model tests
set.seed(42)
ergm.fit00 <- ergm(ergm00)  # Bernoulli model
ergm.fit1 <- ergm(ergm1)    # Full model
...
# Extract BIC and AIC values for models
require(MuMIn)
library(dplyr)
...
uni.distance <- read.csv("distance_matrix.csv", sep=",") # unipartite projection controlling for distance
...
```{r nodes uni, echo= TRUE, message= TRUE, warning= FALSE, fig.width= 9}
...
```{r Bernoulli.uni, echo=TRUE, message= TRUE, warning= TRUE, fig.width=9}
...
```{r models fit uni, echo=TRUE, message= TRUE, warning= TRUE}

uni.ergm00 <- formula(uni.s ~ edges)  

summary.statistics(uni.ergm00) # 1478
           
set.network.attribute(uni.s, "edge_weights", uni.weights) 
set.network.attribute(uni.s, "distance", uni.distance)

uni.ergm1 <- formula(uni.s ~ edges  
                        + nodemain("Plant_ID")
                        + nodemain("Max_height_m")
                        #+ nodemain("Mean_height_m")
                        #+ nodemain("Area_m2")
                        ...
                        #+ nodemain("Seed_mass"))
                        #+ nodemain("Fitness"))
                        + nodemain("Fitness_norm") + edgecov("distance"))
...
```{r model tests uni, echo=TRUE, message= FALSE, warning= TRUE}

# Model tests
# 
set.seed(42)
uni.ergm.fit00 <- ergm(uni.ergm00)  # Bernoulli model
uni.ergm.fit1 <- ergm(uni.ergm1)    # Full model
